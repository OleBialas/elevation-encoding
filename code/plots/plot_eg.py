...
x = np.array([-50., -25.,   0.,  25.,  50.]) # plot all lines over this interval
...
for label, axes in zip(['A', 'B', 'C'], ax):
    ...
    axes.text(0.0, 1.0, label, transform=axes.transAxes + trans,
...
for eg, axes in zip(mean_eg, ax):
    ...
    trans = mtransforms.ScaledTranslation(10/72, -5/72, fig.dpi_scale_trans)
    axes.text(0.6, 0.1, label, transform=axes.transAxes + trans,
            fontsize='medium', verticalalignment='top', fontfamily='serif')
